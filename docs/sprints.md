...
10. **Sprint 10 – Trazabilidad unificada y auditoría** (Completado)
   - Objetivos: consolidar trazabilidad end-to-end por lote, producto y cliente; exponer consultas históricas separadas por flujos inbound, movimientos internos y outbound; habilitar guías operativas y tableros para investigaciones rápidas.
   - Criterios de aceptación: búsquedas por `batchCode`, `productId` o `customerId` devuelven recibos, movimientos, órdenes, handling units y embarques relacionados; el historial de producto separa inbound/outbound/movimientos/inventario; se documentan pasos operativos y visualizaciones para auditar eventos.
