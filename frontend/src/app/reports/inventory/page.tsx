...
export default function InventoryReportPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => get<ReportRow[]>("/reports/inventory", filters),
    enabled: allowed,
    staleTime: 30_000
  });
    ...
    <AppShell>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <ExportButton data={exportRows} filename="reporte-inventario" disabled={!rows.length} />
        </div>

        <FilterBarAdvanced storageKey="report-inventory" onChange={(f) => setFilters(f)} />

        <Card>
          <CardHeader className="flex items-center justify-between">
            <CardTitle>Stock por SKU/ubicación</CardTitle>
